...
function gateKeeper(req, res, next) {
  ...
  if(req.get('x-username-and-password')) {
    let {user, pass} = queryString.parse(req.get('x-username-and-password'));
    USERS.find(validUser=>{return((validUser.userName===user) && (validUser.password===pass)&&(req.user = user));});
  }
  next();
}
